#!/bin/bash
# ============================================================
#  BX6 Solutions — Dylan Autonomous Prime Installer
...
echo "  ╚═════╝ ╚═╝  ╚═╝ ╚═════╝      ╚══════╝ ╚═════╝ ╚══════╝ ╚═════╝    ╚═╝   ╚═╝ ╚═════╝ ╚═╝  ╚══╝╚══════╝"
echo -e "${NC}"
echo -e "  ${ORANGE}${BOLD}DYLAN AUTONOMOUS PRIME${NC} ${DIM}·${NC} Full Stack Autonomous Agent ${DIM}·${NC} ${GREEN}$499${NC}"
echo -e "  ${DIM}OpenClaw + Kimi K2.5 (Free) + WhatsApp + n8n + Firewall + Auto-Restart${NC}"
echo ""
echo -e "  ${DIM}────────────────────────────────────────────────────────────${NC}"
br
...
read -p "  $(echo -e ${CYAN})Agent name${NC} [Dylan]: " AGENT_NAME
AGENT_NAME=${AGENT_NAME:-Dylan}
...
SYSTEM_PROMPT="You are $AGENT_NAME, a fully autonomous AI revenue agent built and deployed by BX6 Solutions. You run 24/7 without supervision. Your goals are: (1) reply to all client communications within minutes, (2) proactively find and close new revenue opportunities, (3) automate repetitive workflows, (4) self-repair any issues before they affect clients, (5) research anything a client asks using your browser. You are professional, concise, and results-focused. You never say you cannot do something — you find a way or ask for the minimum information needed."
...
cat > /etc/systemd/system/dylan.service <<EOF
[Unit]
...
Restart=always
RestartSec=5
StandardOutput=append:/var/log/dylan.log
StandardError=append:/var/log/dylan.log
...
systemctl enable dylan.service &>/dev/null
systemctl start dylan.service &>/dev/null || true
ok "Systemd service: dylan.service enabled"
ok "$AGENT_NAME restarts automatically on crash or reboot"

# ── [7/9] Log Rotation ───────────────────────────────────────
section "[7/9] Log Rotation"

cat > /etc/logrotate.d/dylan <<EOF
/var/log/dylan.log {
  daily
  rotate 7
  compress
...
EOF
ok "Logs: /var/log/dylan.log (7 day rotation)"

# ── [8/9] Auto Backup ────────────────────────────────────────
section "[8/9] Config Backup (Daily)"

BACKUP_DIR="/root/dylan-backups"
mkdir -p "$BACKUP_DIR"

cat > /etc/cron.daily/dylan-backup <<'EOF'
#!/bin/bash
cp ~/.openclaw/openclaw.json /root/dylan-backups/openclaw_$(date +%Y%m%d).json
find /root/dylan-backups -name "*.json" -mtime +7 -delete
EOF
chmod +x /etc/cron.daily/dylan-backup
...
systemctl is-active --quiet dylan.service && ok "dylan.service: running" || warn "Service may still be starting"
...
echo -e "  ${DIM}════════════════════════════════════════════════════════════${NC}"
br
echo -e "  ${GREEN}${BOLD}✓ DYLAN AUTONOMOUS PRIME — INSTALLATION COMPLETE${NC}"
br
...
echo -e "  ${CYAN}Logs:${NC}            tail -f /var/log/dylan.log"
echo -e "  ${CYAN}Restart:${NC}         systemctl restart dylan"
echo -e "  ${CYAN}Status:${NC}          systemctl status dylan"
br
...
echo "$AUTH_TOKEN" > /root/.dylan_token
echo "Token saved to /root/.dylan_token"
br
